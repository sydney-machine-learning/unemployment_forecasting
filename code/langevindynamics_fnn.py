...
# Wil Grebner and Rohitash Chandra, Centre for Translational Data Science
# University of Sydey, Sydney NSW, Australia.  2017 wilgrebner@gmail.com
...
class MCMC:
    ...
    def sampler(self, w_limit, tau_limit):
        ...
        ## these were commented when changed
        #works well
        #step_w = 0.005;  # defines how much variation you need in changes to w
        #step_eta = 0.01; # exp 0
        #newest
        step_w = 0.005
        step_eta = 0.01
        ...
        for i in range(samples - 1):
            ...
            if u < mh_prob:
                ...
                print  likelihood, prior_current, diff_prop, rmsetrain, rmsetest,'accepted'
...
def main():
    for problem in xrange(8, 9):
        ...
        #defualt Rohitash programmed x = 3
        x = 5
        if x == 3:
            w_limit =  0.02
            tau_limit = 0.2
        ...
        if problem == 8: 
            traindata = np.loadtxt("Data_OneStepAhead/Unem/train.txt")
            testdata = np.loadtxt("Data_OneStepAhead/Unem/test.txt")
        ...
        actual_error = fx_mu-ytestdata
        actual_se = actual_error **2
        actual_mse = np.mean(actual_se)
        actual_mse2 = np.ravel(actual_mse)
        actual_rmse = np.sqrt(actual_mse2)
       
        np.savetxt('mcmcresults/actual_rmse.txt', actual_rmse, fmt='%1.5f')
        np.savetxt('mcmcresults/predictions.txt', fx_mu, fmt='%1.5f')
        np.savetxt('mcmcresults/upperpred.txt', fx_high, fmt='%1.5f')
        np.savetxt('mcmcresults/lowerpred.txt', fx_low, fmt='%1.5f')
